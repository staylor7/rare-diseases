import { hierarchy, group, DSVParsedArray } from "d3";
...
export function draw(csv: DSVParsedArray<Datum>) {
  ...
  const root: ExtendedNode = d3
    .partition<HierarchicalDatum>()
  ...
  function clicked(p: ExtendedNode) {
    parent.datum(p.parent || root);

    root.each(
      (d) =>
        ((d as ExtendedNode).target = {
          x0:
    ...
    path
      ...
      .filter((d: ExtendedNode) => {
        const element = d3.select(d3.selectAll("path").node());
        const fillOpacity: string | null = element.attr("fill-opacity");
      ...
      .attr("fill-opacity", (d) =>
        d.target && arcVisible(d.target) ? (d.children ? 0.6 : 0.4) : 0
      )
      .attr("pointer-events", (d) =>
        d.target && arcVisible(d.target) ? "auto" : "none"
      )
    ...
    label
      .filter(function (d: ExtendedNode) {
        const element = d3.select(d3.selectAll("path").node());
