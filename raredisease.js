...
function graph(data, id){
    ...
    let colors = d3.scaleOrdinal().domain(data) // colorbrewer 2.0, qualitative color set
        .range(["#8dd3c7", "#ffffb3", "#bebada", "#fb8072", "#80b1d3", 
                  "#fdb462", "#b3de69", "#fccde5", "#d9d9d9", "#bc80bd", "#ccebc5", "#ffed6f"], 
                  ["ritu", "indu", "vasu", "rudra", "veda", "aditya", "dishi", "bana", 
                  "bhrama", "netra", "agni", "rishi"]);
                  
    let svg = d3.select(id).append("svg") // create an <svg> element on the webpage
                .attr("width", width)
                .attr("height", height)
    ...
    .join("text")
      ...
      .call(text => text.append("tspan")
          ...
          //.text(d => d.data.gene))
          .text(d => "")) // this should appear on mouseover only, so I've made it blank for now
